...
public class OSGiShim implements AutoCloseable {
	...
	public OSGiShim(Function<Class<?>, BundleContext> bundleContextFunction) {
		System.out.println("callded with function");
...
System.out.println("calleds");
		...
		try {
			System.out.println("regestering ::"+bean);
			System.out.println("service ::"+serviceTypes.toArray(new String[0]));
			 registration = (ServiceRegistration<S>) bundleContextFunction.apply(bean.getClass())
				.registerService(serviceTypes.toArray(new String[0]), bean, getServiceProperties(properties));
		}
		finally {
			System.out.println(registration);
			if (registration != null) {
				System.out.println("Registed {} as {}"+ bean+"  :: "+ registration.getReference());
			}
